...
void Rigidbody2::OnAttach(GameObject* parent)
{
	m_gameObject = parent;
...
void Rigidbody2::Update(float deltaTime)
{
	// update velocity
	...
	//rb_angularVelocity += deltaTime * rb_inertiaTensorInv.MultiplyWithVec(Vector3());
...
void Rigidbody2::Transpose(Vector4 translate)
{
	rb_position += translate;
}

// apply a force on the object
void Rigidbody2::ApplyForce(Vector4 force)
{
	if (rb_mass > 0)
	{
...
void Rigidbody2::ApplyTorque(Vector4 force)
{
	if (rb_mass > 0)
	{
		rb_totalAngularVelocity += force / rb_momentOfInertiaSphere;
	}
}

void Rigidbody2::AddAngularVelocity(Vector4 velocity)
{
	rb_totalAngularVelocity += velocity;
}

void Rigidbody2::AddAngularVelocity(Vector4 velocity, float rotationSpeed)
{
	rb_totalAngularVelocity += velocity;
...
/*void Rigidbody2::UpdateOrientationQuaternion()
{
...
void Rigidbody2::UpdateInertiaTensor()
{
