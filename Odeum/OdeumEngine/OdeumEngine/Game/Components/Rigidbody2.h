...
class Rigidbody2 : public Component
{
public:
	...
	inline Matrix3 GetRotation() { return rb_rotation; }
	...
	inline void SetRotation(Vector4 rotation, float angle) { if (Vector3(rotation).Mag() > 0.0f) rb_rotation = Matrix3(DirectX::XMMatrixRotationAxis(rotation, angle)); } //UpdateTransform(); }
...
private:
	...
	Vector4 rb_position;
	Matrix3 rb_rotation; // the axis in which the object will be rotating around
